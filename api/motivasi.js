...
Pilih 1 ayat Al-Quran ATAU 1 Hadits shahih yang sangat relevan. Balas HANYA dalam format JSON VALID:
{
  ...
  "narrative": "pesan motivasi personal 2-3 kalimat menyebut nama ${nama}"
    ...
    console.log('[GoodDay] Request ke Sumopod untuk', nama);
    ...
    if (!response.ok) {
      ...
      console.error('[GoodDay] Sumopod Error:', response.status, err);
    ...
    if (!jsonMatch) {
      console.error('[GoodDay] No JSON in response:', message.substring(0, 100));
    ...
    console.log('[GoodDay] Success untuk', nama);
    res.setHeader('Cache-Control', 's-maxage=300, stale-while-revalidate');
    res.status(200).json(parsed);

  } catch (error) {
    console.error('[GoodDay] Error:', error.message);
    res.status(500).json({ 
      error: 'Server Error',
